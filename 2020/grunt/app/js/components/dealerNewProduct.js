...
export default class DealerNewProduct {
    constructor(el) {
        this.el = el;
        this.lang = document.getElementsByTagName("html")[0].getAttribute("lang");
        this.dealername = document.getElementsByTagName("html")[0].getAttribute("data-dealername");
        this.shufflerFilter = [];
        this.selectedItems = [];
        this.selectedIds = [];
    ...
    get categoryModels() {
        const modelsArr = [],
            modelTags = Array.prototype.slice.call(this.el.querySelectorAll('.product-listing-component-inner--tag'));
    ...
    get filterItems() {
        return Array.prototype.slice.call(this.el.querySelectorAll(".product-listing-component-inner--filter-item"));
    }

    get resultItems() {
        return Array.prototype.slice.call(this.el.querySelectorAll(".product-listing-component-inner--item"));
    }
    ...
    slideProductCarouselToSelected() {
        const list = this.el.querySelector('.product-listing-component-inner--list');
        const selectedProduct = list.querySelector('.is-selected');
        const slideToIndex = selectedProduct ? [...list.children].indexOf(selectedProduct) : 0;
        const productCarousel = this.el.querySelector(".product-listing-component-inner--list-wrapper").swiper;
    ...
    initProductCarousel() {
        let range = document.createRange();
        range.selectNode(document.body);
        const productsSection = this.el.querySelector(".product-listing-component-inner"),
            ...
            nextPrevButtons = range.createContextualFragment("<div class=\"product-listing-component-inner--nav\"><div class=\"product-listing-component-inner--nav-buttons\"><button class=\"previous\"><i class=\"fa fa-angle-left\"></i></button><button class=\"next\"><i class=\"fa fa-angle-right\"></i></button></div></div>"),
            orginal_html = container.innerHTML,
            new_html = `<div class="product-listing-component-inner--list-wrapper"> 
                      <div class="product-listing-component-inner--list">
                        ${orginal_html} 
                      </div>
                    </div>`,
            that = this;
        ...
        //}
        console.log('[initProdCarousel] ---> showSpinner');
        this.showSpinner();

        this.updateTabClassesOnSecondSlider();

        const allProductSlides = Array.prototype.slice.call(this.el.querySelectorAll(".product-listing-component-inner--item"));
        const productItemWidth = this.el.querySelector(".product-listing-component-inner--item:first-child").offsetWidth;

        let items = allProductSlides,
            itemWidth = (this.el.querySelector(".product-listing-component-inner--list-wrapper").offsetWidth * this.sliderOptions.multiplier);
        ...
        this.el.querySelector(".product-listing-component-inner--list").style.width = ((allProductSlides.length * productItemWidth) + allProductSlides.length) + "px";

        this.productCarousel = new Swiper(".product-listing-component-inner--list-wrapper", {
            slidesPerView: this.sliderOptions.slidesInView,
            slideClass: "product-listing-component-inner--item",
            wrapperClass: "product-listing-component-inner--list",
            nextButton: '.product-listing-component-inner--nav .next',
            prevButton: '.product-listing-component-inner--nav .previous',
            centeredSlides: this.sliderOptions.centered,
            observer: true,
            onImagesReady: () => {
            ...
            for (let s2 in this.allCategorySlides) {
                if (this.allCategorySlides[s2].getAttribute("data-category") === newCategory) {
                    ...
                    if (callback) {
                        ...
                        this.getRangeModels(dealername, newCategory, callback ,modelUid);
                    } else {
                        this.getRangeModels(dealername, newCategory);
                    }
                }
            }
        }
    ...
    updateModelCount() {
        const totalModels = this.el.querySelectorAll(".product-listing-component-inner--item"),
            visibleModels = this.el.querySelectorAll(".product-listing-component-inner--item:not([aria-hidden])").length,
            ...
            resultsHeading = this.el.querySelector(".product-listing-component-inner--heading"),
    ...
    getRangeModels(dealername, uid, callback ,modelUid) {
        const that = this,
            url = `/api/sitecore/dealerproduct/GetMachineList/${this.lang}/${dealername}/${uid}`;
        ...
        this.showSpinner()
            .then(() => {
                ...
                fetch(url, { method: "GET", credentials: 'include' })
                    ...
                    .then(responsehtml => {                        
                        that.renderModels(responsehtml)
                            .then(() => {
                                if (callback) {
    ...
    hideSpinner() {
        return new Promise((resolve, reject) => {
            if (this.loadingCount == 1) {
                ...
                console.log('[hideSpinner] ---> target', target);
    ...
    renderModels(responsehtml) {
        return new Promise((resolve, reject) => {            
            document.getElementById("dealerprodlist").innerHTML = responsehtml;
            resolve(true);
        });
    }

    specificationrow(item) {
        item.foreach()
        return "";
    }

    handleImageLoads() {
        const images = Array.prototype.slice.call(this.el.querySelectorAll(".product-listing-component-inner--item-image img")),
            that = this;
        for (let i in images) {
    ...
    attachFilterEvents() {
        const filters = this.filterItems,
            filterToggle = this.el.querySelector(".product-listing-component-inner--filter-toggle"),
            that = this;
    ...
    selectModelFilter(uid) {
        const targetFilter = this.el.querySelector(`.product-listing-component-inner--filter-item[data-model="${uid}"]`);
    ...
    toggleFilter() {
        const button = this.el.querySelector(".product-listing-component-inner--filter-toggle"),
            target = this.el.querySelector(".product-listing-component-inner--filter");
    ...
    updateCart(event) {
        const target = event.target ? event.target.closest('[data-groups]') : event,
            ...
            name = target.querySelector('.product-listing-component-inner--name').innerText,
    ...
    addToCart(item) {
        ...
        this.el.querySelector('.product-listing-component-inner--list').classList.add('selected');
    ...
    removeFromCart(item) {
        ...
        document.querySelector('.product-listing-component-inner--list').classList.remove('selected');
    ...
    updateTabClassesOnSecondSlider() {
        const categoryType = this.el.querySelector(".jcb-quote-form--categories--item").getAttribute("data-category-type"),
            jcbQuoteFormResults = this.el.querySelector('.product-listing-component-inner');

        jcbQuoteFormResults.className = "product-listing-component-inner"; //remove other classes
