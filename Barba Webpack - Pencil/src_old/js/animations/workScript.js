...
const workScript = (container) => {
    ...
    var $activeSlide = $(container).find('.active'),
        $homeSlide = $(container).find(".slide"),
        $slideNavPrev = $(container).find("#prev"),
        $slideNavNext = $(container).find("#next");
    ...
    function goToNextSlide(slideOut, slideIn, slideInAll) {
        ...
        if (index === size - 1) {
            gsap.to($slideNavNext, 0.3, { autoAlpha: 0.2, ease: 'power4.easeInOut' });
        }
    };
    ...
    function goToPrevSlide(slideOut, slideIn, slideInAll) {
        ...
        if (index === 0) {
            gsap.to($slideNavPrev, 0.3, { autoAlpha: 0.2, ease: 'power4.easeInOut' });
        }
    };
